// imports
import timeConvert from "./time_convert";

// interfaces
import { dateElement } from "../../interfaces/interfaces";

// functions

function convertTest(val: number, from: dateElement, to: dateElement, result: number) {
  return () => {
    expect(timeConvert(val, from, to)).toBeCloseTo(result);
  };
}
